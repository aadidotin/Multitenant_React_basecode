...
const STEPS = [
    ...
    { id: 3, label: 'Workspace', icon: Globe     },
...
const cycleLabel: Record<string, string> = {
    monthly: '/mo', yearly: '/yr', lifetime: ' once',
};
            ...
            {STEPS.map((step, i) => {
                ...
                return (
                    <div key={step.id} className="flex items-center">
                        ...
                        {i < STEPS.length - 1 && (
                            <div
                                className={`mx-1 mb-5 h-px w-12 transition-colors duration-300 ${done ? 'bg-primary' : 'bg-muted-foreground/20'}`}
                            />
                        )}
                    </div>
                );
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function TenantRegister({ packages }: Props) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        company_name:             '',
        subscription_package_id:  null as number | null,
        subdomain:                '',
        email:                    '',
        phone:                    '',
        address:                  '',
    ...
    const canProceedStep2 = data.subscription_package_id !== null;
    const canProceedStep3 = data.subdomain.length >= 3 && subdomainStatus === 'available';
    const canSubmit       = data.email.trim() && data.country.trim() && data.terms;

    const selectedPackage = packages.find((p) => p.id === data.subscription_package_id);
    ...
    return (
        <>
            ...
            <div className="flex min-h-screen items-center justify-center bg-linear-to-br from-slate-50 via-white to-slate-100 p-4">
                ...
                {/* wider on step 2 to fit plan cards side-by-side */}
                <div className={`relative w-full transition-all duration-300 ${step === 2 ? 'max-w-2xl' : 'max-w-lg'}`}>
                    {/* Header */}
                                        ...
                                        <div className="relative">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (
                                                        !data.subdomain ||
                                                        data.subdomain === autoSubdomain(data.company_name)
                                                    ) {
                                                        setData('subdomain', autoSubdomain(e.target.value));
                                                    }
                                                }}
                                                autoFocus
                                            />
                            ...
                            {step === 2 && (
                                <div className="animate-in space-y-5 duration-300 fade-in slide-in-from-right-4">
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-sm text-slate-500">
                                            You'll trial this plan free for 14 days. No credit card needed now.
                                        </p>
                                    </div>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        {packages.map((pkg) => (
                                            <PlanCard
                                                key={pkg.id}
                                                pkg={pkg}
                                                selected={data.subscription_package_id === pkg.id}
                                                onSelect={() => setData('subscription_package_id', pkg.id)}
                                            />
                                        ))}
                                    </div>
                            ...
                            {step === 3 && (
                                <div className="animate-in space-y-5 duration-300 fade-in slide-in-from-right-4">
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-sm text-slate-500">
                                            This is the address your team will use to access the app.
                                        </p>
                                    </div>
                                        ...
                                        <div className="h-5">
                                            ...
                                            {subdomainStatus === 'available' && (
                                                <p className="flex items-center gap-1 text-xs text-emerald-600">
                                                    <Check className="h-3 w-3" /> {data.subdomain}.{CENTRAL_DOMAIN} is available!
                                                </p>
                                            )}
                                            {subdomainStatus === 'taken' && (
                                                <p className="flex items-center gap-1 text-xs text-destructive">
                                                    <AlertCircle className="h-3 w-3" /> This subdomain is already taken.
                                                </p>
                                            )}
                                            {subdomainStatus === 'invalid' && (
                                                <p className="flex items-center gap-1 text-xs text-destructive">
                                                    <AlertCircle className="h-3 w-3" /> Only lowercase letters, numbers, and hyphens allowed.
                                                </p>
                                            )}
                                        </div>
                            ...
                            {step === 4 && (
                                <div className="animate-in space-y-5 duration-300 fade-in slide-in-from-right-4">
                                    <div>
                                        ...
                                        <p className="mt-0.5 text-sm text-slate-500">
                                            We'll send your verification email here.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-1.5 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
                                        ...
                                        <div className="space-y-0.5 text-slate-500">
                                            ...
                                            <p>
                                                Plan:{' '}
                                                <span className="font-medium text-slate-800">
                                                    {selectedPackage?.name} (14-day free trial)
                                                </span>
                                            </p>
                                            <p>
                                                URL:{' '}
                                                <span className="font-medium text-slate-800">
                                                    {data.subdomain}.{CENTRAL_DOMAIN}
                                                </span>
                                            </p>
                                        </div>
                                    </div>
